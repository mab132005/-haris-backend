...
Analyze the following text (SMS, WhatsApp message, or Call transcript) for phishing, scam, bank fraud, OTP theft, fake prize/lottery scams, friend/relative impersonation requesting urgent money transfers/loans, or malicious links.
...
- ONLY classify as fraud (`is_fraud: true`) if the message contains explicit malicious scam intent: fake prize wins requesting money/Vodafone Cash/cards/fees, urgent bank account suspension threat, request for OTP/verification code, friend/relative impersonation asking for urgent money transfer/loan, or phishing links.
...
{{
  ...
  "details": "سبب الخطورة بالتفصيل بالعربية (مثال: تحذير: الرسالة تحوي انتحال صفة صديق وطلب تحويل أموال عاجل طارئ)"
}}"""

    import json

    # 1. محاولة التحليل أولاً عبر Google Gemini API السحابي المباشر (توزيع ذكي للمشغلات)
    if GEMINI_API_KEY:
        for model_name in ["gemini-3.6-flash", "gemini-2.5-flash-lite", "gemini-1.5-flash"]:
            try:
