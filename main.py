...
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "AQ.Ab8RN6ITsbxZA90okhoIn5lN7Fq84rBREg42mFTmqA53Tk7muw")
...
{{
  ...
  "details": "سبب الخطورة بالتفصيل بالعربية (مثال: تحذير: الرسالة تحوي انتحال صفة صديق وطلب تحويل أموال عاجل طارئ)"
    ...
    # 1. محاولة التحليل أولاً عبر Google Gemini API السحابي المباشر 24/7 (دعم API Key و Bearer Token)
    if GEMINI_API_KEY:
        try:
            if GEMINI_API_KEY.startswith("AIzaSy"):
                gemini_url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={AQ.Ab8RN6ITsbxZA90okhoIn5lN7Fq84rBREg42mFTmqA53Tk7muw}"
                headers = {}
            else:
                gemini_url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={AQ.Ab8RN6ITsbxZA90okhoIn5lN7Fq84rBREg42mFTmqA53Tk7muw}"
                headers = {"Authorization": f"Bearer {AQ.Ab8RN6ITsbxZA90okhoIn5lN7Fq84rBREg42mFTmqA53Tk7muw}"}
            ...
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                if resp.status_code == 200:
                    ...
                    print(f"Gemini API Status Code: {resp.status_code} - {resp.text}")
        except Exception as e:
