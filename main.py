...
Analyze the following text (SMS, WhatsApp message, or Call transcript) for phishing, scam, bank fraud, OTP theft, or malicious links.
...
- ONLY classify as fraud (`is_fraud: true`) if the message contains explicit malicious scam intent: urgent bank account suspension threat, request for OTP/verification code, fake prize lottery win, or phishing links.
...
{{
  ...
  "details": "الرسالة عبارة عن محادثة طبيعية وآمنة"
}}"""
    ...
    has_bank_otp = any(w in msg_lower for w in ["otp", "رمز التحقق", "فيزا", "حسابك البنكي", "بطاقة البنك"])
    has_urgent = any(w in msg_lower for w in ["إيقاف", "تجميد", "تحديث بيانات", "مبروك ربحت", "جائزة نقدية"])

    is_fraud = (has_bank_otp and has_urgent) or ("otp" in msg_lower and has_urgent)
    return {
        "is_fraud": is_fraud,
        "confidence_score": 0.90 if is_fraud else 0.95,
        "details": "تحذير: تم اكتشاف مؤشرات احتيال بنكي وتصيد" if is_fraud else "الرسالة تبدو آمنة ومحادثة طبيعية"
    }
