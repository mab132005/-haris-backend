...
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "AQ.Ab8RN6LkA5idxIT6KJpxPnr4Nhi7eb2WPQkfnqkMSWMgLdKi2Q")
  ...
  "details": "سبب الخطورة بالتفصيل بالعربية (مثال: تحذير: الرسالة تحوي انتحال صفة صديق وطلب تحويل أموال عاجل طارئ)"
    ...
    if GEMINI_API_KEY:
        try:
            ...
            data = make_http_post(gemini_url, payload, timeout=15.0)
            if data:
                candidates = data.get("candidates", [])
                if candidates:
                    raw_text = candidates[0].get("content", {}).get("parts", [{}])[0].get("text", "").strip()
                    print(f"✨ [Google Gemini AI Output]: {raw_text}")
