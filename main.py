...
def make_http_post(url: str, json_payload: dict, headers: dict = None, timeout: float = 15.0) -> dict:
    """إرسال طلب HTTP POST باستخدام httpx أو urllib المستقل تلقائياً لضمان عدم حدوث أي ModuleNotFoundError"""
    import json
    headers = headers or {}
    headers["Content-Type"] = "application/json"
    data_bytes = json.dumps(json_payload).encode("utf-8")

    # 1. التجربة بـ httpx أولاً إن كان متوفراً
    try:
        import httpx
        ...
        print(f"httpx notice: {e}")

    # 2. التجربة بـ urllib المدمج في نواة بايثون للضمان الشامل المستقل
    try:
        ...
        print(f"urllib notice: {e}")

    return None

# ==================== 5. تحليل الذكاء الاصطناعي عبر Google Gemini API & Ollama ====================
...
async def analyze_message_ai(req: MessageAnalysisRequest):
    """تحليل فوري لسياق الرسالة والمكالمة باستخدام محرك الذكاء الاصطناعي Google Gemini 3.6 Flash"""
    prompt = f"""You are Haris AI Anti-Phishing Fraud Security Classifier.
Analyze the following text (SMS, WhatsApp message, or Call transcript) for phishing, scam, bank fraud, OTP theft, fake prize/lottery scams, fake parcel delivery scams with payment links, friend/relative impersonation requesting urgent money transfers/loans, or malicious links.
...
- ONLY classify as fraud (`is_fraud: true`) if the message contains explicit malicious scam intent: fake parcel delivery requesting fee/payment/link ('تعذر تسليم شحنتك', 'دفع رسوم الشحن', 'تحديث بيانات الاستلام عبر هذا الرابط'), fake prize wins requesting money/Vodafone Cash/cards/fees, urgent bank account suspension threat, request for OTP/verification code, friend/relative impersonation asking for urgent money transfer/loan, or phishing links.
...
{{
  ...
  "details": "سبب الخطورة بالتفصيل بالعربية (مثال: تحذير: الرسالة تحوي إغراء بشحنة وهمية وطلب دفع رسوم عبر رابط غير رسمي)"
}}"""

    import json

    # 1. محاولة التحليل أولاً عبر Google Gemini API السحابي المباشر 24/7
    if GEMINI_API_KEY:
        for model_name in ["gemini-3.6-flash", "gemini-2.5-flash", "gemini-flash-latest"]:
            try:
                ...
                data = make_http_post(gemini_url, payload, timeout=12.0)
                if data:
                    candidates = data.get("candidates", [])
                    if candidates:
                        raw_text = candidates[0].get("content", {}).get("parts", [{}])[0].get("text", "").strip()
                        print(f"✨ [Google Gemini AI Output ({model_name})]: {raw_text}")
    ...
    # 2. محاولة التحليل ثانياً عبر Ollama Llama 3 المحاسبي المحلي
    for url in ["http://127.0.0.1:11434/api/generate", "http://localhost:11434/api/generate"]:
        try:
            payload = {"model": "llama3", "prompt": prompt, "stream": False}
            data = make_http_post(url, payload, timeout=10.0)
            if data:
                raw = data.get("response", "").strip()
                print(f"🤖 [Llama3 Raw AI Output]: {raw}")
    ...
    # 3. محرك الفحص السريع الاحتياطي التكميلي المعاير بجذور الكلمات العربية
    msg_lower = req.message_body.lower().replace("أ", "ا").replace("إ", "ا").replace("آ", "ا").replace("ة", "ه").replace("ى", "ي")

    # 1. استثناء الإشعارات والربط الرسمي للمعاملات البنكية أو كاش (دفع، تحويل، استعلام رصيد)
    is_official_receipt = ("رصيد حسابك" in msg_lower or "رصيد محفظتك" in msg_lower or "تاريخ العمليه" in msg_lower or "تم تحويل مبلغ" in msg_lower or "تم دفع مبلغ" in msg_lower or "تم ايداع" in msg_lower or "تم خصم" in msg_lower) and not ("otp" in msg_lower or "رمز" in msg_lower or "رابط" in msg_lower or "http://" in msg_lower or "سلف" in msg_lower or "كلمه سر" in msg_lower)

    if is_official_receipt:
        return {
            "is_fraud": False,
            "confidence_score": 0.98,
            "details": "إشعار رسمي آمن بمعاملة أو رصيد محفظة"
        }

    # 2. استثناء النقاشات والمحادثات التي تسرد واقعة احتيال سابقة بدون تهديد مباشر
    is_discussion_or_story = ("بيقولي" in msg_lower or "بيقولوا" in msg_lower or "اتصل بيا وقالي" in msg_lower or "واحد اتصل" in msg_lower or "نرفع التطبيق" in msg_lower or "نصب عليا" in msg_lower) and not ("http://" in msg_lower or "ارسل رمز" in msg_lower or "ارسل ال otp" in msg_lower)

    if is_discussion_or_story:
        return {
            "is_fraud": False,
            "confidence_score": 0.98,
            "details": "محادثة أو سرد لواقعة سابقة ولا تشكل تهديد احتيال مباشر"
        }

    # استثناء الرسائل الترحيبية والبريد الإلكتروني لمنصات الدعم الفني الموثوقة (مثل كاف، التذاكر، الدعم)
    is_legitimate_support_email = ("مرحبا بك" in msg_lower or "انضمامك" in msg_lower or "الاسئله الشائعه" in msg_lower or "تذكره الدعم" in msg_lower or "فريق دعم" in msg_lower) and not ("سدد" in msg_lower or "رسوم" in msg_lower or "دفع" in msg_lower or "تحديث بياناتك" in msg_lower or "ارسل رمز" in msg_lower or "otp" in msg_lower)

    if is_legitimate_support_email:
        return {
            "is_fraud": False,
            "confidence_score": 0.98,
            "details": "رسالة ترحيبية أو إشعار دعم فني موثوق لمنصة خدمات"
        }

    has_impersonation_word = ("سلف" in msg_lower or "صديق" in msg_lower or "هحاسب" in msg_lower or "طارئ" in msg_lower)
    has_money_word = ("محفظ" in msg_lower or "تحول" in msg_lower or "حول" in msg_lower or "كاش" in msg_lower or "فلوس" in msg_lower or "جنيه" in msg_lower or "ريال" in msg_lower or "سلف" in msg_lower)

    is_impersonation_scam = has_impersonation_word and has_money_word

    is_shipping_scam = ("شحن" in msg_lower or "طرد" in msg_lower or "بريد" in msg_lower or "عنوان" in msg_lower) and ("رسوم" in msg_lower or "سداد" in msg_lower or "سدد" in msg_lower or "رابط" in msg_lower or "http" in msg_lower or ".com" in msg_lower or ".net" in msg_lower or "ضغط" in msg_lower or "دخول" in msg_lower)

    is_bank_scam = ("حساب" in msg_lower or "بنك" in msg_lower or "فيزا" in msg_lower or "otp" in msg_lower or "رمز" in msg_lower) and ("ايقاف" in msg_lower or "تجميد" in msg_lower or "تحديث" in msg_lower or "بيانات" in msg_lower or "تاكيد" in msg_lower or "ارسل" in msg_lower)

    is_prize_scam = ("مبروك" in msg_lower or "فزت" in msg_lower or "جايز" in msg_lower or "جائز" in msg_lower) and ("كاش" in msg_lower or "رسوم" in msg_lower or "محفظ" in msg_lower or "كارت" in msg_lower or "تحويل" in msg_lower or "otp" in msg_lower)

    is_fraud = is_impersonation_scam or is_shipping_scam or is_bank_scam or is_prize_scam

    if is_fraud:
        if is_impersonation_scam:
            details = "تحذير: تم رصد محاولة انتحال صفة صديق/قريب وطلب تحويل أموال أو سلف عاجل على المحفظة"
        elif is_shipping_scam:
            details = "تحذير: تم رصد محاولة احتيال شحنات وبريد وهمي وطلب سداد رسوم عبر رابط مزيف"
        elif is_prize_scam:
            details = "تحذير: تم رصد محاولة احتيال جوائز وهمية وطلب تحويل أموال عبر كاش"
        else:
            details = "تحذير: تم رصد محاولة احتيال بنكي وسرقة بيانات حساسة ورمز OTP"
    else:
