...
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "AQ.Ab8RN6KzUo1-5W--UWBgy-969Mb_IO6OQDxjbZ9kahLKoI_8lQ")
...
{{
  ...
  "details": "سبب الخطورة بالتفصيل بالعربية (مثال: تحذير: الرسالة تحوي انتحال صفة صديق وطلب تحويل أموال عاجل طارئ)"
    ...
    # 1. محاولة التحليل أولاً عبر Google Gemini 3.6 Flash API السحابي المباشر 24/7 (استجابة فائقة السرعة < 0.5 ثانية)
    if GEMINI_API_KEY:
        try:
    ...
    # 3. محرك الفحص السريع الاحتياطي التكميلي
    msg_lower = req.message_body.lower()
    is_impersonation_scam = ("سلف" in msg_lower or "مشكلة" in msg_lower or "طارئة" in msg_lower or "صديقك" in msg_lower or "هحاسبك" in msg_lower) and ("تحول" in msg_lower or "فلوس" in msg_lower or "جنيه" in msg_lower or "ريال" in msg_lower or "حالا" in msg_lower or "فودافون" in msg_lower or "رقم" in msg_lower)
    ...
    is_fraud = is_impersonation_scam or is_prize_scam or is_bank_scam or is_shipping_scam

    if is_fraud:
        if is_impersonation_scam:
            details = "تحذير: تم رصد محاولة انتحال صفة صديق/قريب وطلب تحويل أموال عاجلة (احتيال الهندسة الاجتماعية)"
        elif is_prize_scam:
